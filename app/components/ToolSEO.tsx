...
export default async function ToolSEO({ slug, locale, relatedTools = [] }: ToolSEOProps) {
  ...
  const pdfTools = ['pdfMerge', 'pdfSplit', 'pdfCompress', 'pdfToImage', 'pdfRotate', 'pdfOrganize', 'pdfWatermark', 'pdfPageNumber', 'pdfProtect', 'pdfUnlock', 'pdfInfo'];
  ...
  const getToolCategory = (tool: string): string => {
    ...
    if (pdfTools.includes(tool)) return 'pdfTools';
    return 'imageTools';
  };
  ...
  const colorMap: Record<string, string> = {
    ...
    watermark: 'sky',
    grid: 'fuchsia',
    colorExtract: 'emerald',
    eyedropper: 'amber',
    exif: 'purple',
    base64: 'indigo',
    formatConvert: 'yellow',
    compare: 'blue',
    toPdf: 'red',
    bgRemove: 'teal',
    zip: 'blue',
