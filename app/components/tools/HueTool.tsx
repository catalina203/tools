...
export default function HueTool({ children }: { children?: React.ReactNode }) {
  ...
  const download = () => {
    ...
    a.href = resultUrl;
    a.download = `hue-${fileName || 'image.png'}`;
    a.click();
  };
  ...
  const presets = [
    { label: t('imageTools.warm'), value: 30 },
    { label: t('imageTools.normal'), value: 0 },
    { label: t('imageTools.cool'), value: 330 },
  ];
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a1a] transition-colors">
      <nav className="flex items-center justify-between px-8 py-5 max-w-7xl mx-auto border-b border-gray-200 dark:border-white/5">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-lg font-semibold text-gray-900 dark:text-white">{t('imageTools.hue')}</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t('imageTools.hue')}</h1>
          <p className="text-gray-500 dark:text-gray-400">{t('imageTools.hueDesc')}</p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              {!imageSrc ? (
                ...
                <div className="p-6">
                  <div className="bg-gray-900 dark:bg-gray-800 rounded-xl p-6 flex items-center justify-center min-h-[360px]">
                    <img ref={imgRef} src={imageSrc} alt="source" className="max-w-full max-h-[340px] rounded-lg transition-all duration-200" style={{ filter: `hue-rotate(${hue}deg)` }} />
                  </div>
              ...
              <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6 space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('imageTools.hueLevel')}</label>
                    <span className="text-sm font-mono font-bold text-fuchsia-500">{hue}°</span>
                  </div>
                  <input type="range" min={0} max={360} value={hue} onChange={(e) => setHue(Number(e.target.value))} className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full appearance-none cursor-pointer accent-fuchsia-500" />
                  <div className="flex justify-between text-xs text-gray-400 mt-1">
                    <span>{t('imageTools.warm')}</span>
                    <span>{t('imageTools.cool')}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {presets.map((p) => (
                      <button key={p.value} onClick={() => setHue(p.value)} className={`flex-1 py-2.5 text-sm rounded-lg font-medium transition-all border ${hue === p.value ? 'bg-fuchsia-500 text-white border-fuchsia-500 shadow-lg shadow-fuchsia-500/20' : 'bg-white dark:bg-[#1a1a2e] border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-fuchsia-300 dark:hover:border-fuchsia-500/50'}`}>
                        {p.label}
                      </button>
                    ))}
                  </div>
            ...
            {imageSrc && (
              <div className="flex gap-4">
                <button onClick={apply} className="flex-1 py-3.5 bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-400 hover:to-pink-400 text-white rounded-xl font-medium transition-all shadow-lg shadow-fuchsia-500/30 flex items-center justify-center">
                  <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  {t('imageTools.applyHue')}
                </button>
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <svg className="w-5 h-5 mr-2 text-fuchsia-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {resultUrl ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('imageTools.hueHint')}</p>
                </div>
              )}
            </div>
          </div>
