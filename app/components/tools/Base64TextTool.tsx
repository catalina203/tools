...
export default function Base64TextTool({ children }: { children?: React.ReactNode }) {
  ...
  const process = () => {
    ...
    try {
      ...
      setError(t('devTools.base64DecodeError'));
      setOutput('');
    }
  };
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a1a] transition-colors">
      <nav className="flex items-center justify-between px-8 py-5 max-w-7xl mx-auto border-b border-gray-200 dark:border-white/5">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-lg font-semibold text-gray-900 dark:text-white">{t('devTools.base64Text')}</span>
        </div>
        <div className="flex items-center space-x-3"><LanguageSwitcher /><ThemeToggle /></div>
      </nav>

      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="mb-8"><h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t('devTools.base64Text')}</h1><p className="text-gray-500 dark:text-gray-400">{t('devTools.base64TextDesc')}</p></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
              ...
              <div className="flex items-center space-x-3">
                <label className="flex items-center space-x-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-lg cursor-pointer hover:border-green-300 dark:hover:border-green-500/50 transition-colors">
                  <input type="radio" name="mode" value="encode" checked={mode === 'encode'} onChange={() => setMode('encode')} className="w-5 h-5 text-green-500 border-gray-300 focus:ring-green-500" />
                  <span className="text-gray-700 dark:text-gray-300">{t('devTools.encode')}</span>
                </label>
                <label className="flex items-center space-x-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-lg cursor-pointer hover:border-green-300 dark:hover:border-green-500/50 transition-colors">
                  <input type="radio" name="mode" value="decode" checked={mode === 'decode'} onChange={() => setMode('decode')} className="w-5 h-5 text-green-500 border-gray-300 focus:ring-green-500" />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6"><h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('devTools.tips')}</h3><ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400"><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.base64Tip1')}</li><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.base64Tip2')}</li><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.base64Tip3')}</li></ul></div>
          </div>
        </div>
