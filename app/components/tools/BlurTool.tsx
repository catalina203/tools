...
export default function BlurTool({ children }: { children?: React.ReactNode }) {
  ...
  const download = () => {
    ...
    a.href = resultUrl;
    a.download = `blur-${fileName || 'image.png'}`;
    a.click();
  };
  ...
  const presets = [
    { label: t('imageTools.light'), value: 2 },
    { label: t('imageTools.normal'), value: 6 },
    { label: t('imageTools.heavy'), value: 12 },
  ];
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a1a] transition-colors">
      <nav className="flex items-center justify-between px-8 py-5 max-w-7xl mx-auto border-b border-gray-200 dark:border-white/5">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-lg font-semibold text-gray-900 dark:text-white">{t('imageTools.blur')}</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t('imageTools.blur')}</h1>
          <p className="text-gray-500 dark:text-gray-400">{t('imageTools.blurDesc')}</p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              {!imageSrc ? (
                ...
                <div className="p-6">
                  <div className="bg-gray-900 dark:bg-gray-800 rounded-xl p-6 flex items-center justify-center min-h-[360px]">
                    <img ref={imgRef} src={imageSrc} alt="source" className="max-w-full max-h-[340px] rounded-lg transition-all duration-200" style={{ filter: `blur(${radius}px)` }} />
                  </div>
              ...
              <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6 space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('imageTools.blurLevel')}</label>
                    <span className="text-sm font-mono font-bold text-blue-500">{radius}px</span>
                  </div>
                  <input type="range" min={0} max={20} value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full appearance-none cursor-pointer accent-blue-500" />
                  <div className="flex justify-between text-xs text-gray-400 mt-1">
                    <span>{t('imageTools.none') || '0'}</span>
                    <span>20px</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {presets.map((p) => (
                      <button key={p.value} onClick={() => setRadius(p.value)} className={`flex-1 py-2.5 text-sm rounded-lg font-medium transition-all border ${radius === p.value ? 'bg-blue-500 text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-[#1a1a2e] border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-blue-300 dark:hover:border-blue-500/50'}`}>
                        {p.label}
                      </button>
                    ))}
                  </div>
            ...
            {imageSrc && (
              <div className="flex gap-4">
                <button onClick={apply} className="flex-1 py-3.5 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-400 hover:to-cyan-400 text-white rounded-xl font-medium transition-all shadow-lg shadow-blue-500/30 flex items-center justify-center">
                  <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  {t('imageTools.applyBlur')}
                </button>
            ...
            <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
              ...
              {resultUrl ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('imageTools.blurHint')}</p>
                </div>
              )}
            </div>
          </div>
