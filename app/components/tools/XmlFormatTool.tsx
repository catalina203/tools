...
function formatXml(xml: string, indentSize: number): string {
  ...
  xml.split(/(<[^>]*>)/g).forEach(node => {
    ...
    if (node.match(/^<\w+.*[^/]>$/) || node.match(/^<\w+>$/)) {
      formatted += indentChar.repeat(indent) + node + '\n';
      if (!node.match(/^<\w+.*\/>$/)) {
        indent++;
      }
...
export default function XmlFormatTool({ children }: { children?: React.ReactNode }) {
  ...
  const process = () => {
    ...
    try {
      ...
      setError(t('devTools.xmlFormatError'));
      setOutput('');
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center space-x-3">
                  <label className="flex items-center space-x-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-lg cursor-pointer hover:border-green-300 dark:hover:border-green-500/50 transition-colors">
                    <input type="radio" name="mode" value="format" checked={mode === 'format'} onChange={() => setMode('format')} className="w-5 h-5 text-green-500 border-gray-300 focus:ring-green-500" />
                    <span className="text-gray-700 dark:text-gray-300">{t('devTools.format')}</span>
                  </label>
                  <label className="flex items-center space-x-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-lg cursor-pointer hover:border-green-300 dark:hover:border-green-500/50 transition-colors">
                    <input type="radio" name="mode" value="minify" checked={mode === 'minify'} onChange={() => setMode('minify')} className="w-5 h-5 text-green-500 border-gray-300 focus:ring-green-500" />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-[#1a1a2e] rounded-2xl border border-gray-200 dark:border-white/10 p-6"><h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('devTools.tips')}</h3><ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400"><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.xmlFormatTip1')}</li><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.xmlFormatTip2')}</li><li className="flex items-start gap-2"><span className="text-green-500">•</span>{t('devTools.xmlFormatTip3')}</li></ul></div>
          </div>
        </div>
