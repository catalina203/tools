...
export default function Home() {
  ...
  const favoriteTools = favorites
    .map(key => {
      ...
      if (['crop', 'resize', 'rotate', 'brightness', 'contrast', 'saturation', 'hue', 'grayscale', 'vintage', 'blur', 'sharpen', 'watermark', 'formatConvert', 'compress', 'mosaic', 'grid', 'rounded', 'colorExtract', 'eyedropper', 'exif', 'base64', 'compare', 'bgRemove', 'toPdf'].includes(key)) {
        ...
        category = 'data';
      } else if (['pdfMerge', 'pdfSplit', 'pdfCompress', 'pdfToImage', 'pdfRotate', 'pdfOrganize', 'pdfWatermark', 'pdfPageNumber', 'pdfProtect', 'pdfUnlock', 'pdfInfo'].includes(key)) {
        category = 'pdf';
      }
      return {
        nameKey: key,
  ...
  const categories = [
    ...
    { nameKey: 'pdf', icon: '📄', count: 11, gradient: 'from-red-500 to-rose-500' },
  ];
